...
export default async function TrafficPage() {
  ...
  const [traffic30d, sources, landingPages] = await Promise.all([
    listTrafficDays(supabase),
    listTrafficSources(supabase),
    listLandingPages(supabase),
  ]);

  const totalSessions = traffic30d.reduce((a, d) => a + d.sessions, 0);
  const totalConv = traffic30d.reduce((a, d) => a + d.conversions, 0);
  const avgBounce =
    traffic30d.length > 0
      ? traffic30d.reduce((a, d) => a + d.bounceRate, 0) / traffic30d.length
      : 0;
      ...
      <div className="p-6 max-w-[1400px] space-y-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <SummaryTile
            ...
            value={formatPercent(
              totalSessions > 0 ? totalConv / totalSessions : 0,
              2,
            )}
          />
        ...
        <Card>
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Badge variant="muted">GA4 snapshot</Badge>
          </CardHeader>
          <CardContent>
            <MiniTrafficChart data={traffic30d} />
          </CardContent>
        </Card>
