...
const SEVERITY_TONE: Record<AuditSeverity, { ring: string; dot: string; label: string }> = {
  ...
  warning: { ring: 'ring-amber-500/40', dot: 'bg-amber-400', label: 'Warning' },
    ...
    <>
      ...
      <div className="p-6 max-w-[1400px] space-y-5">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <SummaryTile label="Critical" value={counts.critical} tone="text-red-400" />
          <SummaryTile label="Warnings" value={counts.warning} tone="text-amber-400" />
          <SummaryTile label="Info" value={counts.info} tone="text-sky-400" />
          <SummaryTile label="Passing" value={counts.pass} tone="text-emerald-400" />
        </div>
          ...
          {sorted.map((f) => {
            ...
            return (
              <li
                key={f.id}
                className={cn(
                  'rounded-lg border border-evari-edge bg-evari-surface p-5 ring-1 ring-inset',
                  tone.ring,
                )}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {f.severity !== 'pass' && f.recommendation && (
                      <div className="mt-3 rounded-md border border-evari-edge bg-evari-carbon p-3">
                      ...
                      <div className="mt-3 flex gap-1.5 flex-wrap">
                        {f.affectedUrls.slice(0, 5).map((u) => (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-[11px] font-mono text-evari-dim hover:text-evari-gold inline-flex items-center gap-1 px-2 py-0.5 rounded border border-evari-edge"
                          >
                            {u}
