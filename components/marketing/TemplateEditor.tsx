...
function PreviewModal({ design, brand, onClose }: { design: EmailDesign; brand: MarketingBrand; onClose: () => void }) {
  ...
  async function sendTest() {
    ...
    try {
      // Render with includeFooter:true so the footer renders INSIDE the
      // email's inner content table (where its max-width is enforced).
      // Tell the sender to skip its own footer so we don't get two.
      const sendHtml = renderEmailDesign(design, brand, { device, includeFooter: true });
      const res = await fetch('/api/marketing/templates/preview-send', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ to: testEmail.trim(), html: sendHtml, subject: '[Test] Template preview', skipBrandFooter: true }),
      });
      const data = await res.json().catch(() => ({}));
